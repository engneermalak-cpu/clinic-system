...
async function joinCall() {
    try {
        ...
        localTracks.audioTrack = await AgoraRTC.createMicrophoneAudioTrack({
            encoderConfig: "speech_standard", // أو "speech_standard", "speech_low_quality", "music_standard", "music_high_quality"
        ...
        // ✅ بعد إنشاء track وتفعيله
        const mediaStreamTrack = localTracks.audioTrack.getMediaStreamTrack();
        const usedDeviceId = mediaStreamTrack.getSettings().deviceId;
        await listMicrophones(usedDeviceId); // ← إعادة تعبئة القائمة وتحديد الجهاز الحالي
...
async function listMicrophones(currentDeviceId = null) {
    try {
        ...
        mics.forEach(mic => {
            ...
            if (mic.deviceId === currentDeviceId) opt.selected = true;
